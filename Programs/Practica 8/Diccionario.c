...
DESCRIPCION: Con la implementacion del TADLista realizar la implementacion de una tabla hash abierta, capaz de soportar el
almacenamiento de palabras y sus definiciones (Diccionario de palabras).

OBSERVACIONES: El programa permite poder agregar nuevas palabras, consultar el significado de alguna, modificar el significado
y eliminar palabras del diccionario hash.

COMPILACION:
gcc -o Diccionario.c TADPila(Din|Est).o Diccionario.exe (Si se tiene el objeto de la implementacion)
gcc -o Diccionario.c TADPila(Din|Est).c Diccionario.exe (Si se tiene el fuente de la implementacion)

EJECUCION: Diccionario.exe (En Windows) - ./Diccionario.exe (En Linux)
*/
...
#include<string.h>
#include<stdlib.h>
#include "TADLista.h" //Libreria que contiene las funciones definidas en TADListaDoblementeLigada.c
...
Recibe: Palabra para obtener codigo hash(char palabra[]), tama�o de la tabla o elementos totales como par�metro secundario(int n).
...
/*
int CargarDiccionarioAlf(lista *D,elemento *dicc,unsigned int n);
Descripcion: Funcion que almacena en memoria del programa un diccionario en forma alfab�tica.
Recibe: Diccionario de palabras en forma de elementos(elemento *dicc),
		tama�o de la tabla o cantidad total de palabras(int n).
		apuntador al arreglo de listas donde se almacenar�n elementos(lista *D).
Devuelve:
...
int CargarDiccionarioAlf(lista *D,elemento *dicc,unsigned int n)
{
	...
	for(i=0;i<n;i++)
	{
		...
		ascii=ascii-65;
		if(ascii<0||ascii>25)
			ascii=25; //Si el codigo ASCII no se encuentra entre A & Z se env�a por default a la ultima lista
...
Recibe: Lista donde se almacenar�n los elementos(lista *D),
		arreglo de elementos que contienen palabra y definicion(elemento *dicc),
		tama�o de tabla(unsigned int n).
...
int CargarDiccionarioHash(lista *D,elemento *dicc,unsigned int n)
{
	...
	for(i=0;i<(n*4);i++)	Add(D,e1); //Inicializa la lista con espacios vacios (la lista se inicializa con un tama�o 4 veces mayor al total de elementos)
	p=First(D);
	for(i=0;i<n;i++)
	{
		...
		else if(strcmp(e.palabra,"0")) //Si no es vac�a la posicion (colapso)
		{
...
Recibe: Arreglo de lineas de archivo(linea *e),
		arreglo de elementos donde se almacenar�n las palabras separadas por nombre y definicion,
...
int CargarArchivo(linea *e ,elemento *dicc,lista *L1,lista *L2)
{
	...
	printf("El nombre del archivo hace referencia a un archivo existente en la misma carpeta de almacenamiento del programa.\n");
	printf("El nombre debe escribirse agregando la extension\n");
	printf("Ejemplo: Mi diccionario.txt\n");
	...
	if(n!=0){ //Si se leyeron palabras...
		...
		printf("Archivo leido correctamente. Que metodo de manejo quieres usar?\n");
		printf("1. Almacenamiento alfabeticamente\n");
		printf("2. Almacenamiento hash\n");
		printf("\nIngrese la opcion que desea:[   ]\b\b\b");
...
Devuelve:
Observaciones: Si no ha sido seleccionado un m�todo de almacenamiento, es mostrado un mensaje de error.
*/
int eliminar_palabra(lista *L1,lista *L2)
{
	char palabra[50];
	...
	printf("Ingresa la palabra que quieres eliminar: ");
	fflush(stdin);
	gets(palabra);
	if(opcHash==1) //Si el metodo usado es alfabetico
	{
		...
		if(ascii<0||ascii>25)	printf("Palabra no encontrada.");
		else
		{
			...
			for(i=0;i<Size(&L2[ascii]);i++)
			{
				...
				{
					Replace(&L2[ascii],p,aux); //Se reemplaza el elemento de la lista por uno vac�o.
					i=i+MAX_Pal;
				}
				p=Following(&L2[ascii],p);
			}
			printf("Palabra eliminada correctamente.");//Si se llega al final de la lista y no se encuentra el elemento.
		}
	}
		...
		while(p!=Final(L1)) //Inicia la busqueda hasta el final de la lista hash
		{
			...
			{
				Replace(L1,p,aux); //Se reemplaza el elemento de lista con uno vac�o.
		...
		printf("No se encontro la palabra en la lista"); //Si se llega al final de la lista
	}
	else
		printf("No se han capturado datos\n");
	PAUSE;
	CLEAN;
	return 0;
}
...
int AgregarPalabra(lista *L1,lista *L2)
{
	...
	printf("Ingresa la nueva palabra del diccionario:");
	fflush(stdin);
	gets(palabra);
	...
	else if(opcHash==2)
	{
		...
		printf("No se pudo agregar la palabra.\nNo hay diccionario de referencia.");
	PAUSE;
	CLEAN;
	return 0;
}
...
int ConsultarDefinicion(lista *L1,lista *L2)
{
	...
	printf("Ingresa la palabra que quieres consultar: ");
	fflush(stdin);
	gets(palabra);
	count_jump_query=0;
	if(opcHash==1)//Para almacenamiento alfabetico
	{
		...
		p=First(&L2[ascii]);
		count_jump_query++;
		for(i=0;i<Size(&L2[ascii]);i++,count_jump_query++)//Inicia la busqueda en la lista correspondiente
		{
			e=Position(&L2[ascii],p);
...
int ModificarDefinicion(lista *L1,lista *L2)
{
	...
	posicion p;
	elemento e;
	printf("Ingresa la palabra que quieres modificar: ");
	fflush(stdin);
	gets(palabra);
	count_jump_query=0;
	if(opcHash==1)
	{
		...
		printf("Definicion no encontrada. No se pudo modificar");
	}
	else if(opcHash==2){
		p=First(L1);
		key=hashing(palabra,n); //Llave hash de la palabra
		...
		if(!strcmp(palabra,e.palabra))//Si se encontro la palabra
		{
			...
			printf("Definicion modificada correctamente");
			printf("\nSaltos de busqueda: %d\n",count_jump_query);
		}
		else
		{
